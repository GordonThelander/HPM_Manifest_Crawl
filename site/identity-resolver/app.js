...
function communityCard(app,matchLabel) {
  ...
  const caution=document.createElement('p');caution.textContent=app.lifecycleFlags.length?`Project status: ${app.lifecycleFlags.join(', ')}`:'This result comes from the community wiki. Open its links for current project details.';card.append(caution);
...
function render(result) {
  ...
  if(result.suggestions.length){const cards=heading($('suggestions'),'Possible matches','Grouped by package','suggested');cards.append(groupedPackageCards(result.suggestions));}
  ...
  $('status').textContent=total ? `${authorCount} author, ${namespaceCount} namespace, ${result.exact.length} exact HPM identity, ${result.nameMatches.length} HPM name, ${manualCount} reviewed manual, ${communityCount} wiki listing, ${result.builtIns.length} built-in, ${result.related.length+result.builtInSuggestions.length+result.suggestions.length} other possible result(s).${result.needsNamespace?' Add the namespace to confirm an exact identity.':''}` : 'No match found. Check the spelling or try the author, namespace or package name.';
}
