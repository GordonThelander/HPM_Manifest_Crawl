...
function render(){
  ...
  [...new Set(times.map(time=>new Date(time).getFullYear()))].forEach(year=>{const xx=x(new Date(`${year}-01-01T00:00:00Z`).getTime());if(xx>=left&&xx<=width-right){svg.append(node('line',{x1:xx,y1:top,x2:xx,y2:top+plotHeight,class:'grid'}));const label=node('text',{x:xx,y:height-18,'text-anchor':'middle',class:'axis-label'});label.textContent=year;svg.append(label);}});
