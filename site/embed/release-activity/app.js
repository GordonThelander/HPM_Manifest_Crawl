...
function render(){
  ...
  const width=1100,height=310,left=62,right=26,top=24,bottom=93,plotWidth=width-left-right,plotHeight=height-top-bottom;
  ...
  firstByFamily.forEach((row,family)=>{const xx=x(new Date(row.releasedAt).getTime()),yy=height-48,label=node('text',{x:xx,y:yy,class:'family-label','text-anchor':'end',transform:`rotate(-55 ${xx} ${yy})`});label.textContent=family;svg.append(label);});
