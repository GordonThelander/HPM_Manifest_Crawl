...
<script>
  ...
  function apply(){{const query=search.value.trim().toLowerCase();let shown=0;cards.forEach(card=>{{const type=active==='ALL'||card.dataset.types.split(' ').includes(active);const text=!query||card.dataset.search.includes(query);card.hidden=!(type&&text);if(!card.hidden)shown++;}});const params=new URLSearchParams;if(search.value.trim())params.set('query',search.value.trim());if(active!=='ALL')params.set('type',active);history.replaceState(null,'',`${{location.pathname}}${{params.size?`?${{params}}`:''}}${{location.hash}}`);count.textContent=`Showing ${{shown}} change(s).`;}}
  const requested=new URLSearchParams(location.search);search.value=requested.get('query')||'';if(buttons.some(button=>button.dataset.filter===requested.get('type')))active=requested.get('type');buttons.forEach(button=>{{button.classList.toggle('active',button.dataset.filter===active);button.addEventListener('click',()=>{{active=button.dataset.filter;buttons.forEach(item=>item.classList.toggle('active',item===button));apply();}})}});search.addEventListener('input',apply);apply();
</script>
